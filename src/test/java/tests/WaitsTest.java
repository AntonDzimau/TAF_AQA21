...
public class WaitsTest extends BaseTest {
    ...
    @Test
    public void presenceOfElementTes1() {
        driver.get("http://the-internet.herokuapp.com/dynamic_loading/1");
        ...
        button.click();
        //метод waitForElementInvisible возвращает тру, когда элемент НЕВИДИМ
        //потому в ассерте ТРУ (невиден? Все правильно, тру)
        Assert.assertTrue(wait.waitForElementInvisible(button));

        WebElement loading = wait.waitForVisibilityBy(By.id("loading"));
        //эта проверка уже не нужна, потому что предыдущий метод у вэйта
        //сам проверит, виден ли элемент
        Assert.assertTrue(loading.isDisplayed());
